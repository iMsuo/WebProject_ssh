  ...
  <head>
	...
	<script type="text/javascript" src="js/index.js"></script>
 	<link rel="stylesheet" type="text/css" href="${pageContext.request.contextPath }/css/top.css">
 	 ...
 	 <link rel="stylesheet" href="${pageContext.request.contextPath }/css/showtickets.css" type="text/css"></link>
 </head>
...
<div id="logo"><a href="${pageContext.request.contextPath}"><img src="images/logo.jpg"></img></a></div> 
  <div id="div1"><img src="images/timg.jpg"></img></div><br/>
   ...
   <c:choose>
    ...
    <table>
      <c:forEach items="${tickets}" var="vars" varStatus="sta">
      <tr><td class="pic"><img src="${users[sta.count-1].headerpic}"/></td>
      <td>用户：${users[sta.count-1].username}</td>
      <td class="sub"><a href="${pageContext.request.contextPath}/showticketwords?ticketid=${vars.id}">主题：${vars.subjectcontent}</a></td>
      <td class="content">内容：${vars.words}</td></tr>
      </c:forEach>
    </table>
    <div id="choose">
	     <p> ${page}/${limit}</p>   
		 <span>
		 <c:if test="${page>1}"><a href="${pageContext.request.contextPath}/showtickets?lanmuid=${tickets[1].lanmu.lanMuid}&pageno=${page-1}">上一页</a></c:if>
		                                	
		 <c:if test="${page<limit}"><a href="${pageContext.request.contextPath}/showtickets?lanmuid=${tickets[0].lanmu.lanMuid}&pageno=${page+1}">下一页</a></c:if>
		 </span>
		</div>
    
    
    <div id="formsub">
    <form method="post" action="publishticket">
        <h3>发布主题 </h3><br/>
        <input type="hidden" name="userid" value="${user.id}">
        <input type="hidden" name="lanmuid" value="${tickets[0].lanmu.lanMuid}">
    	标题：<input type="text" name="subjectcontent" placeholder="请填写标题"><br/>
    	内容：<textarea name="words" rows="10" cols="80"></textarea><br/>
